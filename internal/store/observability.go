...
func readObservationTx(ctx context.Context, tx pgx.Tx, workspaceID, key string) (contracts.RunObservation, error) {
	...
	err := tx.QueryRow(ctx, `SELECT id,workspace_id,idempotency_key,payload_hash,schema_version,kind,component,operation,outcome,actor,task_ref,session_ref,causation_id,correlation_id,source_kind,source_id,started_at,finished_at,duration_ms,db_queries,db_rows,input_bytes,output_bytes,input_tokens,output_tokens,total_tokens,usage_measured,usage_estimated,cost_usd,cost_known,retry_count,duplicate_work,artifact_bytes,metadata,evidence FROM fornix.run_observations WHERE workspace_id=$1 AND idempotency_key=$2`, workspaceID, key).Scan(&o.ID, &o.WorkspaceID, &o.IdempotencyKey, &o.PayloadHash, &o.SchemaVersion, &o.Kind, &o.Component, &o.Operation, &o.Outcome, &actor, &task, &session, &o.CausationID, &o.CorrelationID, &o.SourceKind, &o.SourceID, &o.StartedAt, &o.FinishedAt, &o.DurationMS, &o.DBQueries, &o.DBRows, &o.InputBytes, &o.OutputBytes, &o.InputTokens, &o.OutputTokens, &o.TotalTokens, &o.UsageMeasured, &o.UsageEstimated, &o.CostUSD, &o.CostKnown, &o.RetryCount, &o.DuplicateWork, &o.ArtifactBytes, &metadata, &evidence)
	if err != nil {
		return o, err
	}
...
func readCostTx(ctx context.Context, tx pgx.Tx, workspaceID, key string) (contracts.CostLedgerEntry, error) {
	...
	err := tx.QueryRow(ctx, `SELECT id,workspace_id,idempotency_key,payload_hash,schema_version,category,basis,source_kind,source_id,actor,task_ref,session_ref,causation_id,correlation_id,units,unit_cost_usd,amount_usd,amount_known,measured,estimated,input_tokens,output_tokens,duration_ms,bytes,duplicate_work,metadata FROM fornix.cost_ledger WHERE workspace_id=$1 AND idempotency_key=$2`, workspaceID, key).Scan(&c.ID, &c.WorkspaceID, &c.IdempotencyKey, &c.PayloadHash, &c.SchemaVersion, &c.Category, &c.Basis, &c.SourceKind, &c.SourceID, &actor, &task, &session, &c.CausationID, &c.CorrelationID, &c.Units, &c.UnitCostUSD, &c.AmountUSD, &c.AmountKnown, &c.Measured, &c.Estimated, &c.InputTokens, &c.OutputTokens, &c.DurationMS, &c.Bytes, &c.DuplicateWork, &metadata)
	if err != nil {
		return c, err
	}
