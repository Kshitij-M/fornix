...
cli() {
  if [ -x "$repo_root/bin/fornix" ]; then
    ...
    docker run --rm --add-host=host.docker.internal:host-gateway \
      -e FORNIX_URL="$container_url" -e FORNIX_KEY="$key" -e FORNIX_WORKSPACE_ID="$workspace" \
...
proposal=$(cli change propose --repository reference-repo --source-root "$cli_workdir" --type create_file --path "$path" --content "Fornix verified change packet\n" --idempotency "change-smoke:$workspace")
...
duplicate=$(cli change propose --repository reference-repo --source-root "$cli_workdir" --type create_file --path "$path" --content "Fornix verified change packet\n" --idempotency "change-smoke:$workspace")
